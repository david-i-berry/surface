...
,humid_seq_calc AS (
    SELECT
        ...
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day ORDER BY day),
...
,aggreated_data AS (
    SELECT
        ...
        ,year
        ,month
        ,COALESCE(MAX("humid_seq"),0) AS "max_seq"
        ,COUNT(*) FILTER (WHERE is_rh_above) AS "above"
        ,COUNT(*) FILTER (WHERE is_rh_below) AS "below"
...
SELECT
    ...
    ,MAX(CASE WHEN ad.month = 1 THEN
        CASE 
            ...
            ELSE
                CASE product
                    WHEN 'Longest Sequence' THEN ROUND("max_seq"::numeric,2)::text
