...
,humid_data AS (
    SELECT
        ...
        ,month
        ,year
        ,relative_humidity(tmax, tmin) > {{threshold}} AS is_humid_day
        ,relative_humidity(tmax, tmin) > {{threshold}} AS is_rh_above
        ,relative_humidity(tmax, tmin) < {{threshold}} AS is_rh_below
    FROM daily_data
)
,extended_data AS(
    SELECT
...
,humid_seq_calc AS (
    SELECT
        ...
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_jfm ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "JFM_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_fma ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "FMA_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_mam ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "MAM_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_amj ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "AMJ_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_mjj ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "MJJ_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_jja ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "JJA_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_jas ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "JAS_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_aso ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "ASO_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_son ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "SON_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_ond ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "OND_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_ndj ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "NDJ_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_dry ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "DRY_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_wet ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "WET_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_annual ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "ANNUAL_humid_seq"
        ,UNNEST(consecutive_flag_calc(
            ARRAY_AGG(is_humid_day AND is_djfm ORDER BY day),
            ARRAY_AGG(day_gap > 0 ORDER BY day)
        )) AS "DJFM_humid_seq"
