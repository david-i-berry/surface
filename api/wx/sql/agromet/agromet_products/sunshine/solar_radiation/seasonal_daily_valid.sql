...
,cte_calc_1 AS (
    SELECT
        *
        -- Solar declination (d) in radians
        ,0.409 * SIN((2 * PI() / 365 * day_of_year) - 1.39) AS solar_declination
        -- Inverse relative Earth-Sun distance (dr)
        ,1 + 0.033 * COS((2. * PI() / 365.) * day_of_year) AS inverse_relative_earth_sun_distance
        -- Convert latitude to radians
        ,PI() / 180 * latitude AS latitude_radians
    FROM extended_data
)
,cte_calc_2 AS (
    SELECT
        *
        -- Sunset hour angle (ws) in radians
        ,ACOS(-TAN(latitude_radians) * TAN(solar_declination)) AS sunset_hour_angle
    FROM cte_calc_1
)
,cte_calc_3_calc AS (
    SELECT
        *
        ,24 * (60) / PI() * 0.0820 * inverse_relative_earth_sun_distance * (
...
,solar_rad_calc AS (
    SELECT
        *
        -- Inland rs (krs = 0.16)
        ,0.16 * SQRT(max_temp - min_temp) * extraterrestrial_radiation AS rs_inland
        -- To do add an 'is coastal' attribute to wx_station
        -- Coastal rs (krs = 0.19) 
        -- ,0.19 * SQRT(max_temp - min_temp) * extraterrestrial_radiation AS rs_coastal
    FROM cte_calc_3_calc
)
...
,aggreated_data AS (
    SELECT
        st.name AS station
        ,year
        ,ROUND(SUM(CASE WHEN is_jfm THEN rs_inland END)::numeric, 2) AS "JFM_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_fma THEN rs_inland END)::numeric, 2) AS "FMA_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_mam THEN rs_inland END)::numeric, 2) AS "MAM_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_amj THEN rs_inland END)::numeric, 2) AS "AMJ_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_mjj THEN rs_inland END)::numeric, 2) AS "MJJ_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_jja THEN rs_inland END)::numeric, 2) AS "JJA_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_jas THEN rs_inland END)::numeric, 2) AS "JAS_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_aso THEN rs_inland END)::numeric, 2) AS "ASO_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_son THEN rs_inland END)::numeric, 2) AS "SON_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_ond THEN rs_inland END)::numeric, 2) AS "OND_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_ndj THEN rs_inland END)::numeric, 2) AS "NDJ_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_dry THEN rs_inland END)::numeric, 2) AS "DRY_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_wet THEN rs_inland END)::numeric, 2) AS "WET_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_annual THEN rs_inland END)::numeric, 2) AS "ANNUAL_sum"
        ...
        ,ROUND(SUM(CASE WHEN is_djfm THEN rs_inland END)::numeric, 2) AS "DJFM_sum"
