...
-- Allen et al. (1998), FAO-56 (Equation 34).
CREATE OR REPLACE FUNCTION max_sunshine_duration(
    latitude float,          -- degrees (°)
    day_of_year integer      -- (1-366)
) RETURNS float AS $$        -- hours (h)
DECLARE
    solar_declination float; -- radians (rad)
    latitude_radians float;  -- radians (rad)
    sunset_hour_angle float; -- radians (rad)
    day_length_hours float;  -- hours (h)
BEGIN
    solar_declination := 0.409 * SIN((2 * PI() / 365 * day_of_year) - 1.39);
    latitude_radians := PI() / 180 * latitude;
    sunset_hour_angle := ACOS(-TAN(latitude_radians) * TAN(solar_declination));
    day_length_hours := (24 / PI()) * sunset_hour_angle;
    RETURN day_length_hours;
END;
$$ LANGUAGE plpgsql;

-- Allen et al. (1998), FAO-56 (Equation 35).
CREATE OR REPLACE FUNCTION solar_radiation(
    sunshine float,          -- hours (h)
    latitude float,          -- degrees (°)
    day_of_year integer      -- (1-366)
) RETURNS float AS $$        -- MJ/m²/day
DECLARE
    a float DEFAULT 0.25;    -- dimensionless
    b float DEFAULT 0.5;     -- dimensionless
    max_sunshine float;      -- hours (h)
    extraterrestrial_rad float; -- MJ/m²/day
    solar_rad float;        -- MJ/m²/day
BEGIN
    -- FAO-56 (Equation 34).
    max_sunshine := max_sunshine_duration(latitude, day_of_year);

    -- FAO-56 (Equation 21)
    extraterrestrial_rad := extraterrestrial_radiation(latitude, day_of_year);

    -- FAO-56 (Equation 35)
    solar_rad := extraterrestrial_rad * (a + b * (sunshine / max_sunshine));
    RETURN solar_rad;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    solar_rad := solar_radiation(sunshine, latitude, day_of_year);
...
BEGIN
    net_shortwave_rad := net_shortwave_radiation(sunshine, latitude, day_of_year);
    net_longwave_rad := net_longwave_radiation(tmax, tmin, sunshine, latitude, station_elevation, day_of_year);
...
,daily_data AS (
    SELECT
        ...
        ,SUM(CASE WHEN vr.symbol = 'SUNSHNHR' THEN sum_value ELSE NULL END)::float AS sunshine
...
,daily_lagged_data AS (
    SELECT
        *
        ...
        ,solar_radiation(sunshine, latitude, day_of_year) AS solar_rad
        ,net_shortwave_radiation(sunshine, latitude, day_of_year) AS net_shortwave_rad
        ,net_longwave_radiation(tmax, tmin, sunshine, latitude, elevation, day_of_year) AS net_longwave_rad
        ,net_radiation(tmax, tmin, sunshine, latitude, elevation, day_of_year) AS net_rad        
    FROM extended_data
    WHERE year BETWEEN {{start_year}} AND {{end_year}}  
      AND NULL NOT IN (tmin, tmax, sunshine)
)
,aggreated_data AS (
    SELECT
        st.name AS station
...
SELECT
    ...
    ,CASE WHEN "JFM (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "JFM_solar"
    ...
    ,CASE WHEN "FMA (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "FMA_solar"
    ...
    ,CASE WHEN "MAM (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "MAM_solar"
    ...
    ,CASE WHEN "AMJ (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "AMJ_solar"
    ...
    ,CASE WHEN "MJJ (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "MJJ_solar"
    ...
    ,CASE WHEN "JJA (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "JJA_solar"
    ...
    ,CASE WHEN "JAS (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "JAS_solar"
    ...
    ,CASE WHEN "ASO (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "ASO_solar"
    ...
    ,CASE WHEN "SON (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "SON_solar"
    ...
    ,CASE WHEN "OND (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "OND_solar"
    ...
    ,CASE WHEN "NDJ (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "NDJ_solar"
    ...
    ,CASE WHEN "DRY (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "DRY_solar"
    ...
    ,CASE WHEN "WET (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "WET_solar"
    ...
    ,CASE WHEN "ANNUAL (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "ANNUAL_solar"
    ...
    ,CASE WHEN "DJFM (% of days)" >= (100-{{max_day_pct}}) THEN 
        CASE product
            WHEN 'Global Radiation' THEN "DJFM_solar"
...
CROSS JOIN (VALUES ('Global Radiation'), ('Net Shortwave Radiation'), ('Net Longwave Radiation'), ('Net Radiation')) AS products(product)
