...
,aggreated_data AS (
    SELECT
        ...
        ,variable
        ,year
        ,AVG(tsoil) FILTER(WHERE is_jfm) AS "JFM_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_jfm) AS "JFM_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_fma) AS "FMA_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_fma) AS "FMA_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_mam) AS "MAM_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_mam) AS "MAM_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_amj) AS "AMJ_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_amj) AS "AMJ_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_mjj) AS "MJJ_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_mjj) AS "MJJ_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_jja) AS "JJA_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_jja) AS "JJA_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_jas) AS "JAS_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_jas) AS "JAS_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_aso) AS "ASO_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_aso) AS "ASO_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_son) AS "SON_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_son) AS "SON_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_ond) AS "OND_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_ond) AS "OND_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_ndj) AS "NDJ_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_ndj) AS "NDJ_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_dry) AS "DRY_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_dry) AS "DRY_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_wet) AS "WET_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_wet) AS "WET_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_annual) AS "ANNUAL_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_annual) AS "ANNUAL_stdv"
        ...
        ,AVG(tsoil) FILTER(WHERE is_djfm) AS "DJFM_avg"
        ,STDDEV(tsoil) FILTER(WHERE is_djfm) AS "DJFM_stdv"
...
SELECT
    ...
    ,CASE 
        WHEN "JFM_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("JFM_count"::numeric/"JFM_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("JFM_stdv"::numeric, 2)::text
    END AS "JFM stdv"
    ...
    ,CASE 
        WHEN "FMA_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("FMA_count"::numeric/"FMA_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("FMA_stdv"::numeric, 2)::text
    END AS "FMA stdv"
    ...
    ,CASE 
        WHEN "MAM_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("MAM_count"::numeric/"MAM_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("MAM_stdv"::numeric, 2)::text
    END AS "MAM stdv"
    ...
    ,CASE 
        WHEN "AMJ_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("AMJ_count"::numeric/"AMJ_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("AMJ_stdv"::numeric, 2)::text
    END AS "AMJ stdv"
    ...
    ,CASE 
        WHEN "MJJ_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("MJJ_count"::numeric/"MJJ_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("MJJ_stdv"::numeric, 2)::text
    END AS "MJJ stdv"
    ...
    ,CASE 
        WHEN "JJA_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("JJA_count"::numeric/"JJA_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("JJA_stdv"::numeric, 2)::text
    END AS "JJA stdv"
    ...
    ,CASE 
        WHEN "JAS_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("JAS_count"::numeric/"JAS_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("JAS_stdv"::numeric, 2)::text
    END AS "JAS stdv"
    ...
    ,CASE 
        WHEN "ASO_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("ASO_count"::numeric/"ASO_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("ASO_stdv"::numeric, 2)::text
    END AS "ASO stdv"
    ...
    ,CASE 
        WHEN "SON_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("SON_count"::numeric/"SON_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("SON_stdv"::numeric, 2)::text
    END AS "SON stdv"
    ...
    ,CASE 
        WHEN "OND_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("OND_count"::numeric/"OND_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("OND_stdv"::numeric, 2)::text
    END AS "OND stdv"
    ...
    ,CASE 
        WHEN "NDJ_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("NDJ_count"::numeric/"NDJ_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("NDJ_stdv"::numeric, 2)::text
    END AS "NDJ stdv"
    ...
    ,CASE 
        WHEN "DRY_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("DRY_count"::numeric/"DRY_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("DRY_stdv"::numeric, 2)::text
    END AS "DRY stdv"
    ...
    ,CASE 
        WHEN "WET_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("WET_count"::numeric/"WET_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("WET_stdv"::numeric, 2)::text
    END AS "WET stdv"
    ...
    ,CASE 
        WHEN "ANNUAL_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("ANNUAL_count"::numeric/"ANNUAL_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("ANNUAL_stdv"::numeric, 2)::text
    END AS "ANNUAL stdv"
    ...
    ,CASE 
        WHEN "DJFM_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("DJFM_count"::numeric/"DJFM_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("DJFM_stdv"::numeric, 2)::text
    END AS "DJFM stdv"
