...
,daily_data AS (
    ...
    FROM (
        SELECT
            ...
            ,MAX(max_value) AS max_wndpsd
            ,AVG(avg_value) AS avg_wndpsd
        FROM hourly_data
...
--         ,EXTRACT(MONTH FROM day) AS month
--         ,EXTRACT(YEAR FROM day) AS year
--         ,max_value AS max_wndpsd
--         ,avg_value AS avg_wndpsd
...
,aggreated_data AS (
    SELECT
        st.name AS station
        ,year
        ,MAX(max_wndpsd) FILTER(WHERE is_jfm) AS "JFM_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_jfm) AS "JFM_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_fma) AS "FMA_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_fma) AS "FMA_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_mam) AS "MAM_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_mam) AS "MAM_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_amj) AS "AMJ_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_amj) AS "AMJ_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_mjj) AS "MJJ_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_mjj) AS "MJJ_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_jja) AS "JJA_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_jja) AS "JJA_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_jas) AS "JAS_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_jas) AS "JAS_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_aso) AS "ASO_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_aso) AS "ASO_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_son) AS "SON_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_son) AS "SON_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_ond) AS "OND_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_ond) AS "OND_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_ndj) AS "NDJ_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_ndj) AS "NDJ_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_dry) AS "DRY_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_dry) AS "DRY_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_wet) AS "WET_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_wet) AS "WET_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_annual) AS "ANNUAL_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_annual) AS "ANNUAL_avg"
        ...
        ,MAX(max_wndpsd) FILTER(WHERE is_djfm) AS "DJFM_max"
        ,AVG(avg_wndpsd) FILTER(WHERE is_djfm) AS "DJFM_avg"
...
SELECT
    ...
    ,CASE 
        ...
        ELSE ROUND("JFM_max"::numeric, 2)::text
    END AS "JFM max"
    ,CASE 
        WHEN "JFM_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("JFM_count"::numeric/"JFM_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("JFM_avg"::numeric, 2)::text
    END AS "JFM avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("FMA_max"::numeric, 2)::text
    END AS "FMA max"
    ,CASE 
        WHEN "FMA_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("FMA_count"::numeric/"FMA_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("FMA_avg"::numeric, 2)::text
    END AS "FMA avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("MAM_max"::numeric, 2)::text
    END AS "MAM max"
    ,CASE 
        WHEN "MAM_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("MAM_count"::numeric/"MAM_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("MAM_avg"::numeric, 2)::text
    END AS "MAM avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("AMJ_max"::numeric, 2)::text
    END AS "AMJ max"
    ,CASE 
        WHEN "AMJ_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("AMJ_count"::numeric/"AMJ_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("AMJ_avg"::numeric, 2)::text
    END AS "AMJ avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("MJJ_max"::numeric, 2)::text
    END AS "MJJ max"
    ,CASE 
        WHEN "MJJ_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("MJJ_count"::numeric/"MJJ_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("MJJ_avg"::numeric, 2)::text
    END AS "MJJ avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("JJA_max"::numeric, 2)::text
    END AS "JJA max"
    ,CASE 
        WHEN "JJA_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("JJA_count"::numeric/"JJA_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("JJA_avg"::numeric, 2)::text
    END AS "JJA avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("JAS_max"::numeric, 2)::text
    END AS "JAS max"
    ,CASE 
        WHEN "JAS_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("JAS_count"::numeric/"JAS_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("JAS_avg"::numeric, 2)::text
    END AS "JAS avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("ASO_max"::numeric, 2)::text
    END AS "ASO max"
    ,CASE 
        WHEN "ASO_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("ASO_count"::numeric/"ASO_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("ASO_avg"::numeric, 2)::text
    END AS "ASO avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("SON_max"::numeric, 2)::text
    END AS "SON max"
    ,CASE 
        WHEN "SON_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("SON_count"::numeric/"SON_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("SON_avg"::numeric, 2)::text
    END AS "SON avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("OND_max"::numeric, 2)::text
    END AS "OND max"
    ,CASE 
        WHEN "OND_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("OND_count"::numeric/"OND_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("OND_avg"::numeric, 2)::text
    END AS "OND avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("NDJ_max"::numeric, 2)::text
    END AS "NDJ max"
    ,CASE 
        WHEN "NDJ_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("NDJ_count"::numeric/"NDJ_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("NDJ_avg"::numeric, 2)::text
    END AS "NDJ avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("DRY_max"::numeric, 2)::text
    END AS "DRY max"
    ,CASE 
        WHEN "DRY_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("DRY_count"::numeric/"DRY_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("DRY_avg"::numeric, 2)::text
    END AS "DRY avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("WET_max"::numeric, 2)::text
    END AS "WET max"
    ,CASE 
        WHEN "WET_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("WET_count"::numeric/"WET_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("WET_avg"::numeric, 2)::text
    END AS "WET avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("ANNUAL_max"::numeric, 2)::text
    END AS "ANNUAL max"
    ,CASE 
        WHEN "ANNUAL_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("ANNUAL_count"::numeric/"ANNUAL_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("ANNUAL_avg"::numeric, 2)::text
    END AS "ANNUAL avg"
    ...
    ,CASE 
        ...
        ELSE ROUND("DJFM_max"::numeric, 2)::text
    END AS "DJFM_avg"
    ,CASE 
        WHEN "DJFM_max_day_gap" > {{max_day_gap}} THEN 'Gap Exceeded'
        WHEN ROUND(100*("DJFM_count"::numeric/"DJFM_total"::numeric),2) < (100-{{max_day_pct}}) THEN 'Pct Exceeded'
        ELSE ROUND("DJFM_avg"::numeric, 2)::text
    END AS "DJFM avg"
